<div class="rounded-xl border border-gray-100 bg-white p-3 text-right shadow-sm">
    @if($asset->file_path)
    ...
    <div class="mt-3 flex flex-wrap gap-2">
        @can('production_studio.ai_approve')
            <form method="POST" action="{{ route('admin.production-studio.assets.approve', [$project, $asset]) }}">
                @csrf
                <button class="rounded-lg bg-emerald-600 px-3 py-1.5 text-xs font-black text-white">اعتماد</button>
            </form>
            <form method="POST" action="{{ route('admin.production-studio.assets.reject', [$project, $asset]) }}" class="flex gap-1">
                @csrf
        ...
        @if($asset->generationJob)
            @can('production_studio.ai_retry')
                <form method="POST" action="{{ route('admin.production-studio.ai.retry', [$project, $asset->generationJob]) }}">
                    @csrf
