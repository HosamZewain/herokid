                    ...
                    <table class="min-w-[1500px] w-full divide-y divide-gray-100 text-right text-xs">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse($rows as $row)
                                ...
                                @endphp
                                <tr class="align-top">
                                    <td class="px-4 py-4"><p class="font-mono font-black text-gray-900" dir="ltr">{{ $row['short_reference'] ?: $row['key'] }}</p><p class="mt-1 text-[10px] text-gray-400" dir="ltr">{{ $row['created_at']?->timezone(config('app.timezone'))->format('d/m/Y h:i A') }}</p><span class="mt-2 inline-flex rounded-full bg-gray-100 px-2 py-1 font-black text-gray-600">{{ $row['lifecycle_label'] }}</span></td>
