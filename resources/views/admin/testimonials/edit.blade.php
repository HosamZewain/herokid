<x-admin-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <h2 class="text-xl font-bold text-gray-800">تعديل رأي العميل</h2>
        </div>
    </x-slot>
            ...
            <div class="space-y-5">
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="reviewer_name" value="{{ old('reviewer_name', $testimonial->reviewer_name) }}" required
                               class="w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-1">المدينة</label>
                        <input type="text" name="reviewer_location" value="{{ old('reviewer_location', $testimonial->reviewer_location) }}"
                               class="w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="flex items-center gap-3">
                        @if($testimonial->reviewer_avatar)
                            <img src="{{ $testimonial->reviewer_avatar }}" class="w-10 h-10 rounded-full object-cover border" alt="">
                        @endif
                        <input type="url" name="reviewer_avatar" value="{{ old('reviewer_avatar', $testimonial->reviewer_avatar) }}"
                               class="flex-1 rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                    </div>
                    <p class="text-xs text-gray-400 mt-1">مثال: https://i.pravatar.cc/80?img=5</p>
                </div>
                ...
                <div>
                    ...
                    <textarea name="review_text" rows="4" required maxlength="1000"
                              class="w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 text-sm">{{ old('review_text', $testimonial->review_text) }}</textarea>
                </div>

                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-1">التقييم</label>
