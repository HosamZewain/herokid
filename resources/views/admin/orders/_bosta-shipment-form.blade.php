...
<div class="mt-5 rounded-2xl border border-sky-100 bg-sky-50 p-4">
    ...
    @can('bosta.create_shipment')
        <details class="mt-4 rounded-2xl border border-sky-200 bg-white p-4" @if($isRetry || $errors->hasAny(['receiver_name', 'receiver_phone', 'governorate', 'district_name', 'first_line', 'second_line', 'cod_amount', 'order'])) open @endif>
            ...
            <form method="POST" action="{{ route('admin.bosta.shipments.store', $group['representative_id']) }}" class="mt-5 space-y-4">
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    ...
                    <label class="text-xs font-black text-gray-700">المدينة / المنطقة لدى Bosta
                        ...
                        <span class="mt-1 block text-[10px] font-bold text-gray-500">اكتب اسم منطقة معتمد لدى Bosta، مثل ElMaadi بدل اسم المحافظة.</span>
                    </label>
