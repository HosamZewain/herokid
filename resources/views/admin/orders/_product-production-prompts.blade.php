            ...
            <div class="flex flex-col gap-3 px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
                <div class="flex flex-wrap gap-2">
                    @foreach($productProductionPrompts as $productPrompt)
                        @php($promptDomId = $productPrompt['component_count'] === 1 ? 'product-production-prompt-'.$productPrompt['item']->id : 'product-production-prompt-'.$productPrompt['item']->id.'-component-'.$productPrompt['component_key'])
                        <button
                            type="button"
                            data-copy-product-production-prompt-target="{{ $promptDomId }}"
                        ...
                        >
                            نسخ {{ $productPrompt['component_name'] }}
                        </button>
                    @endforeach
                </div>
            ...
            <div class="rounded-2xl border border-fuchsia-100 bg-white p-4 shadow-sm sm:p-6" data-product-production-prompt-card>
                <div class="mb-4 flex flex-col gap-3 border-b pb-3 md:flex-row md:items-center md:justify-between">
                    ...
                    <div class="flex flex-wrap items-center gap-2">
                        ...
                        >
                            نسخ برومبت {{ $productPrompt['component_name'] }}
                        </button>
                    </div>
                </div>
                ...
                <textarea
                    ...
                    data-production-unit-key="{{ $productPrompt['unit_key'] }}"
