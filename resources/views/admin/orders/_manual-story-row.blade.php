...
<article class="rounded-3xl border border-violet-100 bg-violet-50/40 p-4 sm:p-6" data-story-row data-story-index="{{ $index }}">
    ...
    <div class="grid gap-4 md:grid-cols-2">
        <div class="md:col-span-2">
            ...
            <select id="story-{{ $index }}" name="stories[{{ $index }}][story_id]" required class="w-full rounded-xl border-gray-200 bg-white text-right text-sm" data-story-select>
                ...
                @foreach($stories as $story)
                    @php($pricing = $storyPrices[$story->id])
                    <option value="{{ $story->id }}" data-price-cents="{{ (int) round($pricing['effective_price'] * 100) }}" @selected((string) ($row['story_id'] ?? '') === (string) $story->id)>
                        {{ $story->title }} — {{ format_money($pricing['effective_price']) }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="md:col-span-2">
            <label for="child-photos-{{ $index }}" class="mb-1.5 block text-xs font-black text-gray-700">صور الطفل — صورتان أو 3 صور *</label>
            <input id="child-photos-{{ $index }}" name="stories[{{ $index }}][photos][]" type="file" accept="image/jpeg,image/png,image/webp,image/heic,image/heif,.heic,.heif" multiple required class="block w-full rounded-xl border border-dashed border-violet-300 bg-white p-3 text-sm file:ml-3 file:rounded-lg file:border-0 file:bg-violet-600 file:px-4 file:py-2 file:font-black file:text-white" data-photo-input>
            <p class="mt-2 text-xs font-bold text-gray-500" data-photo-names>لم يتم اختيار صور.</p>
