<div class="grid gap-5 sm:grid-cols-2 lg:grid-cols-3" dir="rtl">
    @foreach($packages as $package)
        <article class="group flex h-full flex-col overflow-hidden rounded-3xl border {{ $package->is_featured ? 'border-fuchsia-300 bg-gradient-to-br from-violet-950 to-indigo-950 text-white shadow-xl shadow-indigo-200' : 'border-slate-200 bg-white text-slate-950 shadow-sm' }} text-right transition duration-300 hover:-translate-y-1 hover:shadow-xl">
            @if($package->image_url)
                <a href="{{ route('shop.package.show', $package) }}" class="block aspect-[4/3] overflow-hidden bg-violet-50">
