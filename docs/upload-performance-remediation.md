...
The committed `public/.user.ini` now requests `upload_max_filesize=64M` and `post_max_size=80M`, matching the existing 50 MB single-file Admin attachment contract. In hPanel, verify the effective **web** PHP runtime honors these values; server-level overrides may still apply and PHP may cache `.user.ini` for several minutes. Also verify sufficient execution/input time and writable private storage. The customer cap remains 15 MB per original/derivative. A CLI `php -i` reading is not proof of web PHP settings. No promise is made that raising limits alone improves network bandwidth.
