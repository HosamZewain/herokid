...
class OpenAiTextVisionProvider implements AiTextVisionProvider
{
    ...
    public function extractScenesToJson(ProductionProject $project, AiModel $model, ProductionStoryVersion|string|null $source): StructuredAiResult
    {
        ...
        $prompt = implode("\n", [
            ...
            'The approved child references will supply facial identity later during image generation. Do not copy their clothing, background, pose, or usual expression into scene descriptions.',
            'Visual directions must use the current child name. Clothing, expression, pose, environment, time of day, and lighting must come only from each story scene.',
            'Never describe the child as smiling by default. The facial expression must match the scene emotion and written action.',
            ...
            'Every safe_text_area_notes value must reserve a calm low-detail blank area for later Arabic text overlay inside the same connected scene. Do not ask for generated text, letters, labels, signs, titles, captions, or logos inside the image.',
            'Every scene must be suitable for later print layout on A3 landscape preview artwork while preserving reader-order scene continuity.',
    ...
    public function improveSceneToJson(ProductionProject $project, ProductionScene $scene, AiModel $model): StructuredAiResult
    {
        ...
        $prompt = implode("\n", [
            ...
            'The scene must be designed as one connected A3 landscape two-page reader spread across two facing A4 portrait pages.',
            'Do not describe two separate unrelated illustrations. The artwork must continue naturally from one A4 page to the other.',
            'No Arabic text or any other visible text should be requested inside the image. Reserve calm safe text space only for later layout overlay.',
            'The safe text area must be a quiet low-detail area within the same connected scene, not a generated text box with writing.',
